...
function loadQuranData() {
    ...
    const qNote = localStorage.getItem('quranNote') || "يرجى تحميل المصحف للمتابعة";
    document.getElementById('quran-link').href = qUrl;
    document.getElementById('quran-note').innerText = qNote;
    
    // تعبئة الحقول في لوحة الإدارة عند فتحها
...
function startNewWeek(mode) {
    if (!confirm("بدء أسبوع جديد سيفرض غرامة 50ج على المقصرين. استمرار؟")) return;
...
function renderFinesBoard() {
    document.getElementById('fines-board').innerHTML = `
...
function exportAdminReport() {
    ...
    for (let k in db.tasksStatus) { if (db.tasksStatus[k].h3) sCount++; if (db.tasksStatus[k].rev) rCount++; }
    let csv = "\uFEFFالاسم,الكود,المحفوظ,المراجع,الغرامات\n";
    csv += `"${db.userName}","${db.userId}","${sCount}","${rCount}","${db.fines}"`;
    const link = document.createElement("a");
    link.href = "data:text/csv;charset=utf-8," + encodeURI(csv);
    link.download = `تقرير_${db.userName}.csv`; link.click();
}
